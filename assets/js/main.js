...
// rAF ticking guard already used by the scroll-spy handler above (scrollActiveTicking) and in
// main-alt.js (the homepage's active script); this file (main.js) runs on
// index_with_animation.html and the preview pages, which carry these arrows. The end state each
// frame is identical (timers reset, arrows shown/hidden the same), only the redundant same-frame
// recomputations are dropped.
...
function requestResume() {
    // Guard the #contact section and the form lookups. This action's srStatus block at the end
    // already treats a "contact form absent" page as a no-op, but the scroll, pre-fill and focus
    // below would throw "Cannot read properties of null" before ever reaching that guard if the
    // contact section were removed/commented out. Bail cleanly instead (there's nothing to scroll
    // to, fill, focus, or announce without the form) so the resume link never raises a runtime
    // error. Mirrors main-alt.js (the homepage's active script); this file (main.js) runs on
    // 404.html, index_with_animation.html and the preview pages.
    ...
    // missing region (contact form absent) is simply a no-op. Mirrors main-alt.js (the homepage's
    // active script); this file (main.js) runs on index_with_animation.html and the preview pages,
    // which carry the same "Request resume" button.
...
if (contactForm) {
    ...
    // JS-built live region matches the belt-and-suspenders sr-only utilities used across the site's
    // markup (index.html's .projects-only-h1, weather/'s .visually-hidden, the preview pages) and
    // the identical region already upgraded in main-alt.js (the homepage's active script) — this
    // copy, which runs on index_with_animation.html and the preview pages, was the one sr-only clip
    // still lacking the modern property. Both rules collapse the box to nothing, so the region stays
    // visually hidden and announced to assistive tech exactly as before. No visual change.
